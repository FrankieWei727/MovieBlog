...
import {List, Menu, Avatar, Button} from 'antd'
    ...
    getProfileData = async (v) => {
        try {
            ...
            for (let index = 0; index < response.data.count; index++) {
                temp.push({follower: {username: '', id: index, profile: {bio: '', avatar: ''}}})
            }
            this.setState({
                data: temp
            });
                    ...
                    <List.Item>
                        <div style={{
                            ...
                            <div style={{
                                ...
                                <Link
                                    to={(item.follower.id + '' === this.props.id ? '/profile/' : '/visit/profile/') + (item.follower.id)}>
                                    <Avatar size={35} shape='square' src={item.follower.profile.avatar}/>
                                </Link>
                            </div>
                            <div style={{
                                flexGrow: 1,
                                ...
                                <div style={{
                                    ...
                                    fontSize: '16px',
                                    fontWeight: 'bold'
                                }}>{item.follower.username}</div>
                                <div>{item.follower.profile.bio.slice(0, 12) + '...'}</div>
